...
document.getElementById("verify-pin").addEventListener("click", function () {
  ...
  const typedNumberField = document.getElementById("typed-field");
  const currentFieldNumber = typedNumberField.value;
  const pinSeccuss = document.getElementById("pin-seccuss");
  const pinFailure = document.getElementById("pin-failure");

  if (currentPin === currentFieldNumber) {
    pinSeccuss.style.display = "block";
    pinFailure.style.display = "none";
  } 
  
  else {
    pinFailure.style.display = "block";
    pinSeccuss.style.display = "none";
  }
});
